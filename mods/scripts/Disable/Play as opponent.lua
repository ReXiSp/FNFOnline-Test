---------------Options---------------
activated = true	--change this to false if you want to play without script
doNoteSplashes = true -- change this to active note splash
ShowSide = true -- show shows which side you are at the beginning of the game	
...
HealthCentered = true --leaves health centralized when activating or deactivating the script
...
function onCreatePost()
		makeLuaText('on', 'You Control Opp', 0, 250, 225)
		setTextSize('on', 50)
		setProperty('on.x', 400)
		makeLuaText('off', 'You Control Bf', 0, 250, 225)
		setTextSize('off', 50)
		setProperty('off.x', 400)
...
if activated then
		...
		if ShowSide then
			if getPropertyFromClass('ClientPrefs', 'middleScroll') == false then
				if activated then
							--show your side
					makeLuaText('side', 'your side', 0, 200, 225)
					setProperty('side.borderSize', 1.25)
					setTextSize('side', 50)
					addLuaText('side', true)
							--show bot side
					makeLuaText('sidebot', 'Bot side', 0, 850, 225)
					setProperty('sidebot.borderSize', 1.25)
					setTextSize('sidebot', 50)
					addLuaText('sidebot', true)
				else
					makeLuaText('side', 'your side', 0, 850, 225)
					setProperty('side.borderSize', 1.25)
					setTextSize('side', 50)
					addLuaText('side', true)
							--show bot side
					makeLuaText('sidebot', 'Bot side', 0, 200, 225)
					setProperty('sidebot.borderSize', 1.25)
					setTextSize('sidebot', 50)
					addLuaText('sidebot', true)
				end
			else
				makeLuaText('side', 'your notes', 0, 500, 225)
				setProperty('side.borderSize', 1.25)
				setTextSize('side', 50)
				addLuaText('side', true)
			end
...
function onUpdate(elapsed) 
    ...
    if getPropertyFromClass('flixel.FlxG', 'keys.justPressed.'..activatekey) then
		screenCenter('scoreTxtP2', 'x')
		runTimer('state', 2, 1)
		if HealthCentered then
		...
		if ('camHUD.visible') then
			if getPropertyFromClass('ClientPrefs', 'middleScroll') == true then
				...
				noteTweenX("Mdx8", 3, 745, 0.01, "linear");
						-- bf notes
				setPropertyFromGroup('strumLineNotes', 4, 'x', 400 - 300);
				setPropertyFromGroup('strumLineNotes', 5, 'x', 520 - 290);
				setPropertyFromGroup('strumLineNotes', 6, 'x', 640 + 290);
				setPropertyFromGroup('strumLineNotes', 7, 'x', 760 + 300);
						for i=7,4,-1 do
							noteTweenAlpha('note' .. i .. 'alphatween', i, 0.35, 0.01, 'linear')
						end
							-- your notes alpha
						for i=3,0,-1 do
				...
				setPropertyFromGroup('strumLineNotes', 0, 'x', 400 - 300);
				setPropertyFromGroup('strumLineNotes', 1, 'x', 520 - 290);
				setPropertyFromGroup('strumLineNotes', 2, 'x', 640 + 290);
				setPropertyFromGroup('strumLineNotes', 3, 'x', 760 + 300);
