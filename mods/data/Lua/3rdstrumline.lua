...
function onCreatePost()
	...
	for i=1, #directions do
		if songName == 'chaotic-animals' then
			makeAnimatedLuaSprite('strum'..directions[i], 'noteskins/note_beta', getPropertyFromGroup('opponentStrums', i-1, 'x') + 322 --[[ screenWidth / 2 - 2*177 + 105 * i--]], defaultstrumy)
		else
	...
	for i=0, getProperty('notes.length')-1 do
		if getPropertyFromGroup('notes', i, 'noteType') == 'plr3' then
			...
			if (noteStrumTime > songPosition - safeZoneOffset and noteStrumTime < songPosition + (safeZoneOffset * hitMult)) then
				...
				if songName == 'chaotic-animals' then
					characterPlayAnim('girlfriend', 'sing'..directions[noteData + 1]:upper(), true)
